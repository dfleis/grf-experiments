...
plt <- ggplot(df.Delta, aes(x = threshold, y = Delta, color = id)) + 
  ...
  annotate("point", x = Delta.max["x", "target"], y = Delta.max["y", "target"], color = ggplot2::alpha(my.cols[1], 0.75), size = 3) + 
  annotate("point", x = Delta.max["x", "grad"], y = Delta.max["y", "grad"], color = ggplot2::alpha(my.cols[2], 0.75), size = 3) + 
  annotate("point", x = Delta.max["x", "FPT"], y = Delta.max["y", "FPT"], color = ggplot2::alpha(my.cols[3], 0.75), size = 3) + 
  annotate("point", x = Delta.max["x", "FPT2"], y = Delta.max["y", "FPT2"], color = ggplot2::alpha(my.cols[4], 0.75), size = 3) + 
  scale_color_manual(
    values = my.cols,
...
pdf("figures/example-criteria-comparison.pdf", width = 7, height = 6)
plt
dev.off()